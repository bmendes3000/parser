...
public class ParamsFilterValidator implements Validator {
	...
	public void validate(Object obj, Errors error) {
		...
		if (argsCommand.length < 4) {
			error.reject("Please! send everything arguments to program execution");
			return;
		}
		//get all parameter to validation
		Map<String, Object> values = ArgumentsParams.getParams(argsCommand);
		
		//Check the key access log
		String value = (String) values.get("accesslog");
		if (value.isEmpty()) {
			error.reject("Send path to access log");
		} else if (!new File(value).exists()) {
			error.reject("File Not Found.");
		}
		
		//check start date
		value = (String) values.get("startDate");
		if (value.isEmpty()) {
			error.reject("Send date for filter");
		} else {
			try {
	            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd.HH:mm:ss");
	            Date date = sdf.parse(value);
	            if (!value.equals(sdf.format(date))) {
	            	error.reject("The format date isn't valid");
	            }
	        } catch (ParseException ex) {
	            error.reject("Error on convert the date value");
	        }
		}
		//check duration 
		...
		if (value.isEmpty()) {
			...
			error.reject("The value sent in the duration parameter isn't correct");
		}
		
		//check threshold
		value = (String) values.get("threshold");
		if (new Integer(value).intValue() < 0) {
			error.reject("The value sent in the threshold parameter isn't correct");
		}
